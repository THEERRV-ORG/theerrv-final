...
/**
 * Two lockups, as the brand has: the primary for light grounds and the
 * reversed for ink. Picking by ground rather than recolouring one file means
 * the mark is never tinted, only swapped.
 */
const MARK_SRC = "/logo-theerrv.svg";
const MARK_SRC_INK = "/logo-theerrv-inverse.svg";
  ...
  return (
    <span className={`logo ${onInk ? "logo-on-ink" : ""} ${className ?? ""}`}>
      {mark ? (
        ...
        <img
          src={onInk ? MARK_SRC_INK : MARK_SRC}
          alt=""
          aria-hidden="true"
          className="logo-mark"
        />
