...
export function Rig() {
  ...
  useFrame((_, delta) => {
    ...
    if (portrait) {
      ...
      // Set by width, not by taste: the mark is ~12 world units across and has
      // to sit inside a frame this narrow without clipping either edge. Its
      // vertical placement is handled by the look-at offset below and by the
      // panel's proportional bottom padding — pulling the camera back to fix a
      // vertical problem just shrinks the mark and strands it.
      const pull = 1 + (1 - Math.min(size.width / 900, 1)) * 1.45;
      scratch.pos.multiplyScalar(pull);
      scratch.look.x *= 0.1;
      // Lift the mark clear of the copy, as a fraction of what the camera can
      // actually see rather than a fixed distance in world units. A constant
      // offset is a different proportion of the frame at every camera distance,
      // which is why the mark cleared the nav on a phone but clipped under it
      // on a tablet. 0.11 × distance ≈ 16% of frame height at this field of
      // view, so it lands in the same place on every screen.
